...
def get_dataset_motion_loader(opt, batch_size, shuffle=True):
    ...
    if opt.NAME == 'interhuman':
        ...
        dataloader = DataLoader(dataset, batch_size=batch_size, num_workers=0, drop_last=True, shuffle=shuffle)
    else:
...
def get_motion_loader(batch_size, model, ground_truth_dataset, device, mm_num_samples, mm_num_repeats, shuffle=True):
    ...
    dataset = EvaluationDataset(model, ground_truth_dataset, device, mm_num_samples=mm_num_samples, mm_num_repeats=mm_num_repeats, shuffle=shuffle)
