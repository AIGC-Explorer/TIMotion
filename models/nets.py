...
class TIMotionDenoiser(nn.Module):
    def __init__(self,
        ...
        for i in range(num_layers):
            self.blocks.append(TIMotionTransformerBlock(num_heads=num_heads,latent_dim=latent_dim, dropout=dpr[i], ff_size=ff_size, num_layers=num_layers, cur_layer=i, LPA=kargs['cfg'].get('LPA', False), cfg=kargs['cfg']))
...
class TIMotionDiffusion(nn.Module):
    def __init__(self, cfg, sampling_strategy="ddim50"):
        ...
        self.net = TIMotionDenoiser(self.nfeats, self.latent_dim, ff_size=self.ff_size, num_layers=self.num_layers,
                                    num_heads=self.num_heads, dropout=self.dropout, activation=self.activation, cfg_weight=self.cfg_weight, cfg=cfg)  
